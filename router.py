...
def router_node(state: AgentState) -> dict:
    ...
    prompt = f"""
        Ты — диспетчер ИИ-Справочника Фильмов. Твоя задача — строго определить намерение пользователя и извлечь название или имя.

        ИНСТРУКЦИЯ ПО ВЫБОРУ НАМЕРЕНИЯ (intent):
        - 'add_movie' -> ТОЛЬКО если пользователь хочет ДОБАВИТЬ конкретный ФИЛЬМ/СЕРИАЛ.
        - 'add_actor' -> ТОЛЬКО если пользователь хочет ДОБАВИТЬ конкретного АКТЕРА/АКТРИСУ.
        - 'delete_movie' -> Если нужно УДАЛИТЬ ФИЛЬМ.
        - 'delete_actor' -> Если нужно УДАЛИТЬ АКТЕРА.
        - 'unknown' -> Во ВСЕХ остальных случаях! Если пользователь просит добавить ЖАНР, РЕЖИССЕРА, СТРАНУ и т.д. или просто пишет бред — строго возвращай 'unknown'.

        КРИТИЧЕСКИЕ ПРАВИЛА ДЛЯ entity_name:
        1. Извлекай название фильма или имя строго в том виде и на том языке, как написал пользователь, но переводи в именительный падеж! 
        2. НЕ ПЕРЕВОДИ на английский язык.
        3. Если intent == 'unknown', оставь entity_name пустым.
